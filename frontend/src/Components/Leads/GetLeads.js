...
const GetLeads = () => {
  ...
  const { data: leads, status, error } = useSelector((state) => state.getLeads);
    ...
    <React.Fragment>
      <Card title="Search Lead by ID">
        <Form layout="inline">
          ...
          {status === "succeeded" && leads && !editable && (
            <Form.Item>
          ...
          {status === "succeeded" && leads && editable && (
            <Form.Item>
              <Button type="primary" onClick={() => setEditable(false)}>
                Cancel
              </Button>
            </Form.Item>
          )}
